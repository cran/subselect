...
\item input object of class \texttt{formula};
  \item input object of class \texttt{lda}.
\end{itemize}
...
%  \item[lda\_object] produced as output by the \texttt{lda} command in \texttt{R}'s MASS package
  \item[fitdldamodel] an object of class \texttt{lda}, as produced by the \texttt{lda} command in \texttt{R}'s MASS package.
\end{description}
...
Finally, an example invoking \texttt{ldaHmat} with an \texttt{lda} object as input:

<<>>=
library(MASS)
iris.lda <- lda(x = iris[, -5], group = iris[, 5])
ldaHmat(iris.lda)
@ 
%{\color{red}FALTA um exemplo com input lda}.
