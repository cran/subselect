...
exclude=NULL, include=NULL, improvement=TRUE, printfile=FALSE,
iseed=c(sample(0:4095,3),sample(0:2047,1)*2+1), cooling=0.05, temp=1,
...
# initializations for the Fortran subroutine
        ...
        inc<-c(0,include)
        if (cooling<=0 || cooling >=1) stop("values of cooling must be between 0 and 1")
...
# call to the Fortran subroutine
        ...
        Fortout<-.Fortran("anneal",as.integer(criterio),as.integer(p),
          ...
          as.integer(iseed),as.double(cooling),as.double(temp),
